...
function refreshAP(team){for(const u of M.units)if(u.team===team&&u.hp>0){u.ap=M.AP_MAX;u.freeAvail=true;u.overwatch=false;u.reacted=false;u.bracing=false;if(u.stunned){u.ap=0;u.freeAvail=false;u.stunned=false;}} M.tickCd(team);}   // début de tour : recharges + étourdissement (perte de tour)
